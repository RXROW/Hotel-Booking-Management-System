...
export default function MostPopAds() {
  ...
  const addToFavorite = async (id: string) => {
    try {
      const res = await privateInstance.post(FAVORITE_ROOMS.ADD_TO_FAVORITE, { roomId: id });
      console.log(res)
  ...
  return (
    <Box component="section" sx={{ margin: '0 auto', color: '#152C5B', padding: { xs: '0', md: '20px' } }}>
      <Typography sx={{ fontFamily: 'Poppins', fontWeight: 500, fontSize: '24px', mb: 2 }}>
        Most popular ads
      </Typography>
      ...
      <Box sx={{
        ...
        <Box sx={{
          ...
          {loading ? (
            ...
            <>
              <img
  src={Array.isArray(ads[0]?.room?.images)
...
/>
  ...
  <Box sx={{
    ...
    <Link to={`details-page/${ads[0]?.room._id}`}>
      <RemoveRedEyeOutlinedIcon sx={{ color: '#fff', fontSize: '32px' }} />
    </Link>
  </Box>
          ...
          {ads.slice(1, 5).map((item: adsData, index) => (
            <Box key={index} sx={{
              ...
              {loading ? (
                ...
                <>
                  <img
  src={Array.isArray(item?.room?.images)
...
/>
  ...
  <Box sx={{
    ...
    <Button onClick={() => addToFavorite(item?.room?._id)}>
      <FavoriteIcon sx={{ color: '#fff', fontSize: '28px' }} />
    </Button>
    <Link to={`details-page/${item.room._id}`}>
      <RemoveRedEyeOutlinedIcon sx={{ color: '#fff', fontSize: '28px' }} />
    </Link>
  </Box>
