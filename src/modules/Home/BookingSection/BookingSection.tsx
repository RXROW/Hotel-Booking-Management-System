...
import { Box, Button, Typography, IconButton, TextField, Popover } from "@mui/material";
...
export default function BookingSection(): JSX.Element {
  ...
  const handleDecrease = (): void => setCapacity((prev) => (prev > 1 ? prev - 1 : 1));
...
const onSubmit = () => {
    ...
    if (startDate && endDate) {
      navigate(`/explore-all-rooms?startDate=${startDate.format('YYYY-MM-DD')}&endDate=${endDate.format('YYYY-MM-DD')}&capacity=${capacity}`);
    } else {
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterDayjs}>
      <Box display="flex" flexDirection={{ xs: "column", lg: "row" }} alignItems="center" justifyContent="center" minHeight="100vh" p={4}>
        {/* Left Section */}
        <Box flex={1} pr={{ xs: 0, lg: 4 }} mb={{ xs: 4, lg: 0 }} textAlign={{ xs: "center", lg: "left" }}>
...
/>
          ...
          <Box display="flex" alignItems="center" mb={4} justifyContent="start" gap={0}>
            <IconButton
