...
import {
  ...
  TableFooter, TableHead, TablePagination, TableRow, Paper,
...
const SharedTable: React.FC<SharedTableProps> = ({
  ...
  <TableHead sx={{ backgroundColor: "#E2E5EB", height: "90px", border:"none" }}>
  <TableRow>
      ...
      >
        ...
        <TableBody>
          {loading ? (
            ...
            rows.map((row, idx) => (
              <TableRow key={idx} sx={{ backgroundColor: idx % 2 === 0 ? "white" : "#f9f9f9" }}>
                {columns.map((col) => (
                  <TableCell
                  sx={{border:"none"}}
                    key={col.id}>
