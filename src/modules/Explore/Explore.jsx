...
import { Box, CircularProgress, Typography, Grid, Chip, Breadcrumbs, Link } from "@mui/material";
...
export default function Explore() {
    ...
    const getAvailableRooms = async () => {
        try {
            ...
            const response = await publicInstance.get(ROOMS_URL.GET_ALL_ROOMS_FOR_USER, {
                params: {
                    startDate,
                    endDate,
                },
                ...
                <Grid item xs={12} sm={6} sx={{ textAlign: "right" }}>
                    {startDate && endDate && (
                        <Typography variant="body2" color="text.secondary">
                            Showing availability from {new Date(startDate).toLocaleDateString()} to {new Date(endDate).toLocaleDateString()}
                        </Typography>
                    )}
                </Grid>
            </Grid>
...
const RoomCard = ({ room }) => {
    ...
    return (
        ...
        >
            <img
                src={!imageError && room.images && room.images.length > 0 ? room.images[0] : defaultRoomImage}
                style={{
                    width: "100%",
                    height: "100%",
                    objectFit: "cover",
